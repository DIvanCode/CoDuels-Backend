...
public class StartDuelSearchHandlerTests : ContextBasedTest
{
    ...
    {
        var handler = new StartDuelSearchHandler(Context, new DuelManager());
    ...
    [Fact]
    public async Task Success_when_user_already_in_rated_search()
    {
        var ctx = Context;
        ...
        var handler = new StartDuelSearchHandler(ctx, duelManager);
        ...
        duelManager.GetWaitingUsers().Should().ContainSingle(u =>
            u.UserId == u1.Id && u.ExpectedOpponentId == null);
    }
    ...
    public async Task Success_cancels_friendly_invitation_and_adds_to_rated_search()
    {
        ...
        res.IsSuccess.Should().BeTrue();
        duelManager.GetWaitingUsers().Should().ContainSingle(u =>
            u.UserId == inviter.Id && u.ExpectedOpponentId == null);
