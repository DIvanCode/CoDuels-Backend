...
The `Taski and Exesh` pull-request workflow runs this command once when a PR
changes Taski, Exesh, this scenario, or related root Compose/submodule
configuration. Affected Taski and Exesh production builds depend on this job,
so deployment cannot start after a failed end-to-end test. Keep the scenario's
Compose services, test configs, seeded task, requests, and assertions
synchronized with relevant Taski/Exesh contracts and infrastructure. General
conventions for maintaining and adding scenarios are in
[`../README.md`](../README.md).
