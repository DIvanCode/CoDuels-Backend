...
func (s *OutboxStorage) GetOutboxForProduce(ctx context.Context) (ox *outbox.Outbox, err error) {
	tx := extractTx(ctx)

	ox = &outbox.Outbox{}
	if err = tx.QueryRowContext(ctx, selectOutboxForProduceQuery).
