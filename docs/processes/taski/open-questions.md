...
| File API | File reads now canonicalize requested and metadata paths, use type-specific public-file allowlists for WriteCode/PredictOutput/FindTest, return 400 for invalid paths, 403 for non-public paths, 404 for missing tasks/files, and close readers before unlocking after streaming. The allowlist is still derived from task metadata rather than an independent manifest. | Should public downloadable files be declared in a separate manifest, and what compatibility policy applies if existing task metadata contains malformed public paths? |
