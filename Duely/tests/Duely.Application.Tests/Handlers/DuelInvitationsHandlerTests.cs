...
{
    [Fact]
    public async Task Get_incoming_requests_returns_waiting_users()
    {
        var ctx = Context;
        ...
        var handler = new GetIncomingDuelRequestsHandler(ctx, duelManager);
        var res = await handler.Handle(new GetIncomingDuelRequestsQuery
        {
            UserId = u1.Id
